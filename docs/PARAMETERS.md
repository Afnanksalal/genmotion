...
Layer `bindings` map destination paths such as `text`, `fill`, `src`, `effects.0.amount`, or `tracks.0.keyframes.0.value` to parameter IDs. Array indices must be canonical nonnegative integers. Identity fields, binding definitions, inherited properties, and prototype paths cannot be bound. Values are cloned, and the authored source is preserved separately from the resolved project. Scene preflight bindings can also address transition duration, presentation and overlay composition through explicit safe paths. Composition-instance bindings can supply structured `parameterValues` to a nested reusable definition.
...
Embedded preview accepts the same overrides and named configurations: `player.setParameters(values)`, `player.setVariant(id)`, or HTTP `?variant=ID&parameters=JSON`. Explicit values win over the variant, which wins over active project values/defaults. Frame, metadata, thumbnail and processed audio use the same resolved configuration. Unknown variant IDs are rejected.
