...
- [x] **GM-019** Add local semantic catalog search alongside keyword/tag search with explicit answering tier, model/index version, offline status and scores; pin optional model downloads and keep query text local by default. Evidence: [offline semantic catalog](SEMANTIC-CATALOG.md), local word/character TF-IDF retrieval, per-result scores, explicit tier/algorithm/model/index/offline metadata, no query retention and optional local-model ID/version/SHA-256 verification.
- [x] **GM-020** Detect and report index/registry skew in both directions, unavailable results and unindexed items; refresh safely and explain degraded ranking rather than returning misleading empty results. Evidence: [semantic index diagnostics](SEMANTIC-CATALOG.md), missing-from-index and missing-from-registry reports, unavailable-result separation, deterministic ranking and atomic full-index refresh tests.
...
- [x] **GM-023** Maintain local search-miss and content-quality records that users can inspect or delete; any external report must be a separate explicit action, not background telemetry. Evidence: [local catalog feedback](CATALOG-FEEDBACK.md), strict bounded records, atomic host-selected storage, complete inspection, selective/full deletion and explicit payload-only export with no network or background telemetry path.
...
- [x] **GM-078** Support JSON/JSONL/CSV batch inputs with row-level validation, deterministic IDs/output names, concurrency limits, progress, isolated failure and selective retry without rerendering accepted rows. Evidence: [deterministic batch rendering](BATCH-RENDERING.md), typed parameter validation for all three formats, content identities, portable collision-safe outputs, bounded worker pool, per-row progress and durable receipts, isolated failures, changed-row detection and accepted-row skip tests.
- [x] **GM-079** Reuse content-addressed project uploads and assets across render jobs and providers with integrity checks, deduplication, quotas and deterministic local dependency resolution. Evidence: [shared content-addressed inputs](CONTENT-STORE.md), SHA-256 staging/readback verification, collision-safe immutable storage, byte/entry quotas, cross-job deduplication, exclusive local materialization and deterministic project/dependency identities retaining original roles.
...
- [x] **GM-086** Generate focused capability documentation, examples and schemas from canonical contracts; detect drift between Studio controls, MCP/CLI/SDK support and bundled skills. Evidence: [generated capability contracts](CAPABILITY-CONTRACTS.md), typed canonical operation registry, generated Markdown/machine registry/per-operation JSON Schemas/executable request examples, exact artifact checks, and drift diagnostics across SDK exports, CLI commands, MCP tools, Studio routes/data and bundled skill guidance in the mandatory repository gate.
- [x] **GM-087** Package small domain/workflow skills with explicit input/output contracts, lazy capability loading, resumable state and executable examples; do not require all knowledge in every agent turn. Evidence: [lazy domain skills](DOMAIN-SKILLS.md), four packaged focused domains, strict capability/input/output manifests, trigger-ranked bounded selection, native Codex skill attachments, selected-only Claude/Hermes context, atomic revision/hash-bound step state and handler-executed example validation.
- [x] **GM-088** Check project/runtime/library version compatibility before upgrades; preserve pins, run migrations and representative validation, and provide rollback with a visible version/change report. Evidence: [version-safe upgrades](UPGRADES.md), runtime/schema/library-pin inspection, explicit future-version refusal, bounded v0→v1 migration, dry run, complete semantic validation, first/middle/last native-frame evidence, immutable backups, confined atomic rollback and version/hash/change reports across SDK, CLI, MCP and Studio.
...
- [x] **GM-092** Represent each supplied reference with a frozen source record: content hash, original location, owner/rights status, permitted use, redistribution status, attribution text, and the user's authorization or unresolved-rights warning. Block export presets that conflict with a recorded restriction; unknown rights must remain visible rather than becoming an implied license. Evidence: [supplied reference rights](REFERENCE-RIGHTS.md), strict portable source/authorization schemas, mandatory unknown-rights warnings and native pre-render enforcement of purpose and redistribution restrictions.
- [x] **GM-093** Build a reversible reference-preparation graph that preserves raw media and records every extracted frame, audio stream, crop, matte, cleaned plate, measurement and generated derivative with parent hashes, tool/version/settings, exact source-time mapping and declared retained/replaced regions. Evidence: [reversible reference preparation](REFERENCE-PREPARATION.md), strict content-addressed DAG, complete preparation-kind inventory, parent/cycle validation, tool/settings/time/region provenance, file verification and dependent-branch removal tests.
- [x] **GM-094** Support a declared adaptation map that links reference intervals and measurements to native scene/layer targets, marks intentional differences, and distinguishes original native layers, captured evidence, source-derived pixels and generated replacements in Studio, bundles and delivery manifests. Evidence: [declared reference adaptation](REFERENCE-ADAPTATION.md), strict map/target/region schemas, all four pixel-origin categories, cross-reference and native-target validation, preparation assets in portable dependency closure, Studio inspection API and hash-bound origin summaries in every render manifest.
- [x] **GM-095** Analyze reference timing, motion landmarks, camera/cursor positions and visual regions into reviewable bounded observations with confidence and manual correction; native reconstruction must consume those observations through stable declarative tracks rather than executable source-project code or renderer-time inference. Evidence: [reviewable reference observations](REFERENCE-OBSERVATIONS.md), bounded frozen measurement inputs, deterministic peak/geometry observation generation, per-observation confidence and analyzer identity, hash-linked manual corrections, explicit limitations/truncation and dimension-aware compilation into ordinary seek-safe native tracks.
- [x] **GM-096** Produce reference-versus-output comparisons at declared aligned timestamps with optional exclusion masks for intentional changes, per-region image metrics, contact sheets and boundary samples. A mismatch report must separate reference-preservation defects from intended adaptation differences and retain the compared source and output hashes. Evidence: [reference comparison evidence](REFERENCE-COMPARISON.md), bounded exact-frame extraction, normalized regional/exclusion geometry, MAE/RMSE/PSNR/change metrics, retained frame samples, reference/output/difference sheets, boundary indexes, separate preservation/intentional-difference records and source/output SHA-256 tests.
- [x] **GM-097** Require an explicit, hash-bound acceptance record before downloading a restricted font or other gated asset. Verify the downloaded bytes, refuse silent substitutions or changed upstream bytes, and record the terms URL, accepted version, actor and time without treating a generic project save as consent. Evidence: [restricted asset acceptance](RESTRICTED-ASSETS.md), strict portable acceptance records, distinct explicit creation API, staged host-owned download, SHA-256 verification, atomic publication and changed-byte/substitution refusal tests.
- [x] **GM-098** Generate a render-input attestation from the resolved dependency graph. It must identify all render inputs and their roles, prove that delivered/comparison exports are excluded as inputs, verify required frozen assets before rendering, and retain encoded-output, decoded-video and audio-stream hashes after a full decode. Evidence: [render attestation](RENDER-ATTESTATION.md), role-tagged dependency closure, declared-hash enforcement, self-input rejection, staged full-stream RGBA/PCM decode hashes and atomic acceptance tests through the shared CLI, MCP, Studio and SDK renderer.
