...
## Capability and refusal query

`canApplySemanticEdit` is a pure query over the current source revision. Alongside the operation-specific allow/refuse result, it reports whether the stable target resolved, is locked, inherited through an instance, marked as frozen imported content, has parameter bindings or dependants, and requires materialization. A schema-derived control list states which semantic operations are enabled and gives a concrete reason for disabled controls. Studio, SDK, CLI session commands, and MCP session commands consume this same result; the query never changes history or persists source. Asset and evaluated-frame validation remains explicitly deferred to the transaction.

Every mutation has explicit lifecycle receipts. A `dispatched` event identifies the source revision, origin, and stable affected targets before persistence begins. The returned `validated`, `saved`, or `verified` receipt carries `changed`, before/after revisions, inverse patch, validation findings, readback state, and any frame evidence. Refused proposals and persistence failures expose a typed failure receipt through the error details with `changed: false`, no after revision, target/evidence fields, and a stage-specific code. Process completion is therefore never presented as proof that source was saved or output was visually verified.
