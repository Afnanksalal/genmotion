...
QA found and fixed oversized MCP schema responses, unwired gesture controls, a preview-image content-policy restriction, floating-point scene-boundary drift, live-context polling during agent work, and browser-default Studio widgets. Browser fixture cleanup retries transient Windows file locks; timeline tests wait for the rendered canvas target before dragging it. Cross-platform CI also exposed compact-toolbar clipping and expected context conflicts surfacing as failed polling requests. Compact desktop layouts now use two toolbar rows; a dedicated context-sync endpoint reports an explicit applied/not-applied outcome and preserves strict conflict errors for editing commands. Regression tests verify stale polling cannot overwrite authoritative context. Build and browser verification run sequentially because a build replaces served distribution files.
