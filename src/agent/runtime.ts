...
function requestsFullTimelineReview(prompt: string): boolean {
  return /\b(?:render|export|full (?:video|film|timeline)|contact sheet|transition boundar(?:y|ies)|final encode)\b/i.test(prompt);
}
...
function buildPrompt(input: AgentRunInput): string {
  ...
  const changeRequested = requestRequiresProjectChange(input.prompt);
  const fullReviewRequested = requestsFullTimelineReview(input.prompt);
  return [
    ...
    '',
    'Apply the request to the real Genmotion project when it asks for a change. Use the genmotion MCP tools when available. Before the first write in a new ACP authoring session, call genmotion_schema and genmotion_project_read; never infer the Creative IR shape from memory. In later turns of the same session, reuse the schema already returned and call only genmotion_project_read unless a field is genuinely unclear. The save tool also exposes the complete authoritative document schema inline. Text layers require x, y, width, height, fontFamily, fontSize, and color. For every layer, x/y are the top-left of its layout box, not its center; text align and verticalAlign position content inside that box, while anchorX/anchorY only define the transform pivot. Layer transform.x/transform.y are additional offsets and normally start at 0. Never duplicate layer x/y into transform x/y. Direct transform tracks animate offsets around 0, not absolute canvas positions. A track keyframe at is layer-local time from 0 through layer.duration, not scene time and not global composition time. Direct tracks require id, target, and keyframes with at, value, and ease; cubic-bezier easing is an object with type, x1, y1, x2, and y2. Build a schema-valid static scene system first, save it, then add complex motion through revision-safe patches so one bad track cannot discard the whole composition. Validate and inspect rendered frames. The Creative IR is an open authoring surface: design original scenes, direct property tracks, custom cubic-bezier or spring timing, and SVG path geometry. Named recipes are optional references, never a required template. Preserve truthful product evidence, local asset provenance, reproducible frame evaluation, existing brand decisions, and unrelated human edits. Do not create HTML, Remotion, HyperFrames, placeholder copy, fake product behavior, or unsourced claims. Do not commit, publish, install packages, access credentials, or use the network. For a visual change, inspect at least one representative native frame before finishing, and inspect each affected scene or transition when the scope spans several. Do not render the full video, create contact sheets, or start servers unless the user explicitly asks to export or review the full timeline.',
