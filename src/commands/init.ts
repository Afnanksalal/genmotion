...
export interface InitOptions { title: string; promise: string; proof: string; desiredAction: string; audience: string; mode: CreativeBrief['mode']; duration: number; width?: number; height?: number }
...
export async function initializeProject(directory: string, options: InitOptions): Promise<{ directory: string; projectFile: string; briefFile: string }> {
  ...
  const project = projectSchema.parse({
    ...
    scenes: [{
      ...
      layers: [{
        id: 'artboard', type: 'shape', shape: 'rect', x: 0, y: 0, width, height,
