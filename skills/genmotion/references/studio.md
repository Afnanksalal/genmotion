...
- The inspector exposes the complete production controls for every Creative IR layer family, including clipping, cropping, shadows, text layout, vector paths, source playback, blend modes, anchors, audio behavior, inbound and outbound transitions, and named, cubic-bezier, or spring easing. Text and vector layers can be created without an agent.
