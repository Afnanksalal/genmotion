...
describe('workflow CLI surfaces', () => {
  it('executes caption delivery, music planning, presentation validation, and audio timing from packaged commands', async () => {
    ...
    const audioTiming = JSON.parse((await run(process.execPath, [path.resolve('dist/cli.js'), 'audio-timing', project, '--json'])).stdout) as { version: number; tracks: unknown[] }; expect(audioTiming).toMatchObject({ version: 1, tracks: [] });
  });
});
