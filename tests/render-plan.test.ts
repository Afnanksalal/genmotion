...
describe('deterministic render plan', () => {
  it('resolves delivery metadata and hashes frozen dependencies', async () => {
    ...
    expect(first.preflight).toMatchObject({ width: 1920, height: 1080, fps: 30, duration: 6 });
    expect(first.selection.range).toEqual({ startFrame: 0, endFrame: 180 });
    expect(first.variantValues).toMatchObject({ accent: '#ffc857' });
    ...
    expect(first.dependencies).toEqual([expect.objectContaining({ path: 'assets/Inter.ttf', bytes: 876576, hash: expect.stringMatching(/^[a-f0-9]{64}$/) })]);
  ...
  it('plans selected ranges and rejects impossible delivery contracts', async () => {
    ...
    expect(selected.selection).toMatchObject({ sceneId: 'typed-output', range: { startFrame: 90, endFrame: 180 } });
    expect(selected.delivery).toMatchObject({ dimensions: { width: 1920, height: 1080 }, color: { alpha: { mode: 'preserve' } }, output: { filename: 'scene.webm' } });
